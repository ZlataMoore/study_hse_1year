...
std::map<std::string, double> MakeMap(std::string_view words) {

    std::map<std::string, double> result;
    int64_t begin_word = 0;
    int64_t length_of_word = 0;
    int64_t size_of_words = words.size();
    for (int64_t index = 0; index != size_of_words; ++index) {
        if ((index == 0) || (std::isalpha(words[index]) && std::isalpha(words[index - 1]))) {
            ...
            std::string_view word = words.substr(begin_word, length_of_word);
            std::string word1;
...
std::vector<std::string_view> Search(std::string_view text, std::string_view query, size_t results_count) {
    ...
    std::map<std::string, double> idf = MakeMap(query);

    std::vector<std::pair<std::pair<int, int>, std::map<std::string, double>>> help_vector;
    ...
    int64_t size_of_text = text.size();
    for (int64_t index = 0; index != size_of_text; ++index) {
        ...
        if (text[index] == end) {
            ...
            help_vector.push_back(string_tf_pair);
            tf_for_string.clear();
    ...
    for (int64_t index = 0; index != static_cast<int64_t>(help_vector.size()); ++index) {
        double relevance_measure = 0;
        for (const auto &word : help_vector[index].second) {
            relevance_measure += static_cast<double>(word.second) * log(idf[word.first]);
        }
        auto string_measure = std::make_pair(help_vector[index].first, relevance_measure);
    ...
    for (size_t index = 0; index != fmin(results_count, measures_for_strings.size()); ++index) {
